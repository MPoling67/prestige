...
// ─────────────────────────────────────────────────────────────────────────────
// Built strictly against KoT-design-system-skill__1_.md
// Light mode throughout (--bg #f4f3ef, Plus Jakarta Sans body)
// Three-tier banner: dark shelf → light tool panel → newsletter (after results)
// ─────────────────────────────────────────────────────────────────────────────
...
function PulseLoader({ text }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "4px 0" }}>
      ...
      <span style={{ fontSize: 12, color: "#6b6b66", fontStyle: "italic", fontWeight: 300 }}>{text}</span>
    </div>
  );
}
...
function ScoreBar({ score, max }) {
  ...
  return (
    <div style={{ background: "#edecea", borderRadius: 2, height: 3, overflow: "hidden" }}>
      <div style={{ height: "100%", width: `${pct}%`, background: col, borderRadius: 2, animation: "kot-bar 1.2s ease forwards" }} />
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f4f3ef", color: "#1a1a18" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,300;0,9..144,600;1,9..144,300;1,9..144,600&family=Plus+Jakarta+Sans:wght@300;400;500&display=swap');
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── Tier 1: Brand shelf — always dark ── */
        .kot-tier1 { display: flex; overflow: hidden; }
        .kot-tier1-strip { width: 7px; background: #861442; flex-shrink: 0; }
        .kot-tier1-bar {
          flex: 1; background: #1a120e;
          padding: 7px 1.5rem; display: flex; align-items: center;
        ...
        /* ── Tier 2: Tool panel — light ── */
        .kot-tier2 { display: flex; overflow: hidden; }
        .kot-tier2-strip { width: 7px; background: #861442; opacity: 0.35; flex-shrink: 0; }
        .kot-tier2-inner {
        ...
        .kot-headline em { font-style: italic; color: #be3650; }
        .kot-tagline {
          font-family: var(--font-body);
          font-size: 12px; color: var(--muted); margin-top: 5px;
        }

        /* ── Tier 3: Newsletter — only after results ── */
        .kot-tier3 { display: flex; overflow: hidden; }
        .kot-tier3-strip { width: 7px; background: #861442; opacity: 0.2; flex-shrink: 0; }
        .kot-tier3-inner {
          flex: 1; background: #1a120e;
          border: 1px solid rgba(255,255,255,0.08); border-top: none;
          padding: 9px 1.5rem; display: flex; align-items: center; gap: 12px; flex-wrap: wrap;
        }
        .kot-nl-label {
          font-family: var(--font-body);
        ...
        .kot-nl-input {
          ...
          padding: 6px 12px; outline: none; font-family: var(--font-body);
        }
        .kot-nl-input::placeholder { color: rgba(255,255,255,0.2); }
        .kot-nl-btn {
          background: #861442; color: #ffffff; border: none; border-radius: 5px;
          font-family: var(--font-body); font-size: 11px; font-weight: 500;
          letter-spacing: 0.06em; padding: 7px 16px; cursor: pointer;
        ...
        .kot-input-zone {
          background: var(--bg);
          padding: clamp(28px,5vw,44px) clamp(16px,4vw,2rem) clamp(32px,5vw,48px);
          border-bottom: 1px solid var(--border);
        }
        .kot-dek {
          font-family: var(--font-body); font-size: 14px; font-weight: 300;
          color: var(--text); line-height: 1.7; margin-bottom: 22px; max-width: 560px;
        }
        .kot-pills { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 28px; }
        .kot-pill {
        ...
        .kot-pill em { font-style: normal; color: #be3650; margin-right: 4px; }
        .kot-input-label {
          font-family: var(--font-body); font-size: 11px; letter-spacing: 0.12em;
          text-transform: uppercase; color: var(--text); margin-bottom: 10px;
        }
        .kot-input-row { display: flex; gap: 10px; flex-wrap: wrap; max-width: 600px; }
        .kot-url-input {
        ...
        .kot-url-input:focus { border-color: #861442; }
        .kot-url-input::placeholder { color: var(--muted); }

        /* ── Buttons ── */
        .btn-primary {
          background: #861442; color: #ffffff; border: none;
        ...
        .kot-report-zone {
          ...
          padding: 0 clamp(16px,4vw,2rem) 80px;
        }
        ...
        .kot-report-date {
          ...
          letter-spacing: 0.12em; text-transform: uppercase; color: #be3650;
        }
        ...
        .card-label {
          ...
          color: #861442; margin-bottom: 14px;
        }
        .card-body {
        ...
        .narrative-box {
          background: var(--surface); border: 1px solid var(--border);
          border-left: 3px solid #861442;
          border-radius: 0 var(--radius) var(--radius) 0;
          padding: clamp(14px,3vw,20px) clamp(16px,3vw,24px);
          font-family: var(--font-body); font-size: 14px;
          line-height: 1.8; color: var(--text); font-weight: 300; margin-bottom: 14px;
        }

        /* ── Score number ── */
        .kot-score-num {
          font-family: var(--font-display); font-weight: 300; font-style: italic;
          font-size: clamp(72px,13vw,108px); line-height: 1; letter-spacing: -0.04em;
        }
        .kot-score-den {
          font-family: var(--font-display); font-size: 22px;
          font-weight: 300; color: var(--muted); padding-bottom: 8px;
        }
        ...
        .page-footer a { color: #be3650; text-decoration: none; }
        ...
        @media (max-width: 480px) {
          .kot-input-row, .kot-email-row { flex-direction: column; }
          .kot-input-row .btn-primary { width: 100%; text-align: center; }
          .kot-tier2-inner, .kot-input-zone { padding-left: 1rem; padding-right: 1rem; }
        }
      `}</style>

      {/* ── Tier 1: Brand shelf ── */}
      <div className="kot-tier1 no-print">
        <div className="kot-tier1-strip" />
        <div className="kot-tier1-bar">
          <span className="kot-tier1-label">Knowledge on Tap</span>
        </div>
      </div>

      {/* ── Tier 2: Tool panel ── */}
      <div className="kot-tier2 no-print">
        <div className="kot-tier2-strip" />
        <div className="kot-tier2-inner">
          <h1 className="kot-headline">POWER <em>Score</em></h1>
          <p className="kot-tagline">Free AI-powered business audit · monicapoling.com</p>
        </div>
      </div>
      ...
      <div className="kot-input-zone no-print">
        <div style={{ maxWidth: 820, margin: "0 auto" }}>
          ...
          <div className="kot-pills">
            {POWER_SECTIONS.map(({ letter, label }) => (
              <div key={letter} className="kot-pill">
                <em>{letter}</em>{label}
              </div>
            ))}
          </div>
          ...
          {error && (
            <div style={{ marginTop: 16 }}>
              <p style={{ color: "#c0705a", fontSize: 13, fontFamily: "var(--font-body)", marginBottom: 8 }}>{error}</p>
              {debugInfo && (
                <>
                  <button className="btn-ghost" onClick={() => setDebugOpen(o => !o)} style={{ fontSize: 11, padding: "5px 12px" }}>
        ...
        <div className="kot-report-zone" style={{ animation: "fadeUp 0.5s ease both" }}>
          ...
          <div className="card kot-anim" style={{ animationDelay: "0.1s" }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 8 }}>
              <span className="kot-score-num" style={{ color: overallColor }}>{sc}</span>
              <span className="kot-score-den">/100</span>
            </div>
            <p style={{ fontFamily: "var(--font-body)", fontSize: 12, color: "var(--muted)", letterSpacing: "0.06em", marginBottom: 16 }}>
              {playbook.overallDescriptor}
            </p>
            <div style={{ background: "#edecea", borderRadius: 2, height: 3, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${sc}%`, background: overallColor, borderRadius: 2, animation: "kot-bar 1.2s ease forwards" }} />
            </div>
          </div>
              ...
              {["Industry-leading competitors", "Trends to watch", "Three moves worth making"].map((label) => (
                <div key={label} className="card kot-anim">
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, paddingTop: 4 }}>
                    ...
                    <span style={{ fontFamily: "var(--font-body)", fontSize: 13, color: "var(--muted)", fontWeight: 300 }}>
          ...
          {!emailSubmitted && (
            <div className="card kot-anim no-print" style={{ animationDelay: "0.4s" }}>
              ...
              <div className="kot-email-row" style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 10 }}>
              ...
              <p style={{ fontFamily: "var(--font-body)", fontSize: 11, color: "var(--muted)", marginBottom: 16 }}>
          ...
          {emailSubmitted && (
            <p style={{ fontFamily: "var(--font-body)", fontSize: 13, color: "#0F6E56", marginBottom: 14 }}>
          ...
          {emailSubmitted && (intelLoading || intel || intelError) && (
            <div className="card kot-anim">
              ...
              {intelError && <p style={{ fontFamily: "var(--font-body)", color: "#c0705a", fontSize: 13 }}>{intelError}</p>}
          ...
          {emailSubmitted && (revenueLoading || revenue || revenueError) && (
            <div className="card kot-anim">
              ...
              {revenueError && <p style={{ fontFamily: "var(--font-body)", color: "#c0705a", fontSize: 13 }}>{revenueError}</p>}
              {revenue?.moves?.map((m, i) => (
                <div key={i} className="intel-row">
                  <p style={{ fontFamily: "var(--font-body)", fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "#861442", marginBottom: 6 }}>
                    Move {i + 1}
                  </p>
              ...
              {revenue?.closingLine && (
                <p style={{ fontFamily: "var(--font-body)", fontSize: 13, color: "#861442", fontStyle: "italic", marginTop: 16 }}>
                  {revenue.closingLine}
                </p>
              )}
            </div>
      ...
      {/* ── Tier 3: Newsletter — only after results ── */}
      {playbook && (
        <div className="kot-tier3 no-print">
          <div className="kot-tier3-strip" />
          <div className="kot-tier3-inner">
            <span className="kot-nl-label">Let's Make Some Noise</span>
            <input type="email" placeholder="your@email.com" className="kot-nl-input" />
            <button className="kot-nl-btn">Subscribe →</button>
          </div>
        </div>
      )}
      ...
      <footer className="page-footer">
        ...
        <p>Knowledge on Tap | <a href="https://monicapoling.com" target="_blank" rel="noopener noreferrer">Monica Poling</a></p>
      </footer>
